...
3.  **Logistic Regression (`glm`):** Logistic regression, a foundational
...
4.  **Deep Nueral Netweork (DNN):** This is a a powerful machine
...
5.  **Gradient Boosting (`gbm`):** Gradient boosting is another machine
    learning algorithm that can be adapted for survival analysis. It
...
# Train the GBM model with AdaBoost
#gbm_model <- gbm(COD ~ ., data = train_data, distribution = "bernoulli", n.trees = 100, 
#                 interaction.depth = 1, shrinkage = 0.01, bag.fraction = 0.5, 
#                 train.fraction = 1.0, cv.folds = 5, verbose = TRUE)

# Make predictions on the test set
predictions_gbm <- predict(gbm_model, newdata = test_data, n.trees = 100, type = "response")

# Extract predicted probabilities for class 1
predicted_probabilities <- predictions_gbm[, 1, 1]

# Convert predicted probabilities to class labels
predicted_class <- ifelse(predicted_probabilities > 0.5, 0, 1)
...
heatmap <- ggplot(heatmap_data, aes(x = Prediction, y = Reference, fill = Freq)) +
  ...
  theme_minimal() +
  geom_text(aes(label = Freq), color = "black", size = 3) +  # Add text labels
  ggtitle("GBM Predictive Model") +  # Add title
...
# Calculate AUC ROC
roc_curve <- roc(test_data$COD, predicted_probabilities)
print(roc_curve)
...
Cox proportional hazards regression, often referred to as Cox regression
or Cox model, is a widely used statistical method for analyzing the
relationship between the survival time of subjects and their predictor
variables. It's particularly useful for studying the time until a
particular event occurs, such as death or disease recurrence.

In R, you can fit a Cox proportional hazards model using the coxph
function from the survival package. This function takes a formula
argument specifying the survival time and predictor variables, and it
estimates the regression coefficients using partial likelihood
estimation. Cox proportional hazards regression in R, implemented with
...
c_index_result <- rcorr.cens(predicted_risk, test_data_1$Time, test_data_1$Status)
c_index <- c_index_result[["C Index"]]
...
# Plot Kaplan-Meier survival curves
surv_plot <- ggsurvplot(survfit(Surv(Time, Status) ~ 1, data = test_data_1), data = test_data_1,
...
# Make predictions on the test set (compute predicted survival probabilities)
predicted_survival <- survfit(model_coxph, newdata = test_data_1)

#ggsurvplot(predicted_survival, data = test_data_1)

surv_prob_at_50_months <- summary(predicted_survival, times = 50)$surv
...
CoxPH and DNN. Overall, the project demonstrated that even individuals
with limited domain knowledge can utilize available resources to predict
...
RandomForest. Other methods such as GBM and SVM proved challenging and
demanded a deeper understanding of the methodology. Additionally, neural
networks exhibited success but were time-consuming and presented
black-box risks. For future iterations, I would opt to focus on Logistic
Regression and RandomForest, dedicating more time to encoding, data
preparation, and exploring stratification and parameter stress testing
to potentially enhance accuracy.

This project explored the feasibility of predicting breast cancer
patient survival rates using machine learning. While achieving a target
accuracy of over 96% proved challenging, a Logistic Regression model
achieved a promising 93% accuracy on data spanning 2011-2015 from the
SEER database (300,000+ cases).

The project successfully identified 16 critical factors influencing
survival from a pool of 36 through exploratory data analysis. Machine
learning techniques (data cleaning, feature engineering) were
implemented using R packages to prepare the data for various algorithms,
including Logistic Regression, Random Forests, Cox Proportional Hazards
(CoxPH), and Deep Neural Networks (DNNs).

The analysis excluded the initially considered "Other" outcome category
due to its limited relevance to breast cancer-specific survival.
